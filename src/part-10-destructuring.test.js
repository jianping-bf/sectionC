...
test("infer how varargs work in JS using the rest operator with function parameters", () => {
  ...
  // todo ...
  const concat = ([...args]) => args[0] + args[1] + args[2];
  let array = ["-", "!", "-"];
  let result = concat(array);
...
test("show that the destructuring can be done to any level", () => {
  let system = {
    planets: [
  ...
  // todo ...
  //let { planets: [...args] } = system;
  let {
    planets: [, , { mass: earthMass }]
  } = system;
  //let earthMass = args[2].mass;
